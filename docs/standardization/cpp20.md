...
### メンバアクセス演算子の仕様を一貫性のために修正 [(P0704R1)](http://wg21.link/p0704r1)

C++17 までは、右辺値オブジェクトに対してメンバアクセス演算子 `.*` で「左辺値参照修飾されたメンバ関数ポインタを呼ぶことは不適格」という規格文面になっていました。そのため、同じ意味をもつ次の 2 つのプログラムで後者だけ不適格とされ、一貫性がありませんでした。
```C++
#include <iostream>
#include <string>
...
```
C++20 では「左辺値参照修飾された非 const メンバ関数ポインタを呼ぶことは不適格」と文面を修正し、後者もコンパイルできるようになります。
